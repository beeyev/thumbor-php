...
{
    public function testItChecksIfAddingFiltersIsWorkingCorrectly(): void
    {
        $thumbor = (new Thumbor())->imageUrl('abc.jpg')->addFilter('round_corner', '20%7C20', 0, 0, 0);
        self::assertSame('unsafe/filters:round_corner(20%7C20,0,0,0)/abc.jpg', $thumbor->get());

        $thumbor->addFilter('brightness', 5)->addFilter('blur', 7)->addFilter('autojpg');
        self::assertSame('unsafe/filters:round_corner(20%7C20,0,0,0):brightness(5):blur(7):autojpg()/abc.jpg', $thumbor->get());

        $thumbor->noFilter();
        self::assertSame('unsafe/abc.jpg', $thumbor->get());

        $thumbor = (new Thumbor())->imageUrl('abc.jpg')->addFilter('noise', 40);
        self::assertSame('unsafe/filters:noise(40)/abc.jpg', $thumbor->get());
    }
}
